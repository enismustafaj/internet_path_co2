...
if __name__ == '__main__':
    ...
    for i, site in enumerate(sites):
        ...
        geolocations = []
        for hop in hops:
            geolocations.append(utils.get_location_from_ip(constants.IP_DATA_ENDPOINT % hop, None, {
                                'api-key': os.getenv('IP_DATA_API_KEY')}))

        carbon_intensities = []

        for i, location in enumerate(geolocations):
        	carbon_intensities.append(utils.get_carbon_intensity(constants.CO2_SIGNAL_ENDPOINT, headers={'auth-token': os.getenv('CO2_SIGNAL_API_KEY')}, params={'lat':location[0], 'lon':location[1]}))
